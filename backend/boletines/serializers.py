...
class BoletinSerializer(serializers.ModelSerializer):
    ...
    # Representación del grado y nivel (Ej: "1er Año A - secundaria")
    ...
    def get_generado_por_nombre(self, obj):
        ...
        if obj.generado_por:
            nombre = getattr(obj.generado_por, 'first_name', None) or getattr(obj.generado_por, 'nombre', '')
            apellido = getattr(obj.generado_por, 'last_name', None) or getattr(obj.generado_por, 'apellido', '')
    ...
    def validate(self, data):
        ...
        """
        # Obtenemos los valores del data (si es creación) o de la instancia (si es actualización)
        ...
        # Validación de unicidad solo en creación
        if not self.instance:
            if Boletin.objects.filter(
                ...
                raise serializers.ValidationError(
                    f"Ya existe un boletín generado para este estudiante en el lapso {lapso} del periodo {periodo}."
                )
        return data