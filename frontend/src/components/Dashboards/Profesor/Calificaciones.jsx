...
export function Calificaciones() {
    ...
    const cargarCalificaciones = async () => {
        try {
            const response = await axiosInstance.get(`calificaciones/?materia=${selectedMateria}&lapso=${selectedLapso}`);
            ...
            response.data.forEach(cal => {
                ...
                calificacionesData[key] = {
                    id: cal.id,
                    notas: [
                        cal.nota1 !== null && cal.nota1 !== undefined ? parseFloat(cal.nota1) : '',
                        cal.nota2 !== null && cal.nota2 !== undefined ? parseFloat(cal.nota2) : '',
                        cal.nota3 !== null && cal.nota3 !== undefined ? parseFloat(cal.nota3) : '',
                        cal.nota4 !== null && cal.nota4 !== undefined ? parseFloat(cal.nota4) : ''
                    ],
    ...
    const handleNotaChange = (estudianteId, notaIndex, value) => {
        const nota = value === '' ? '' : Math.max(0, Math.min(20, parseFloat(value) || 0));
        
        setCalificaciones(prev => {
            const key = `${estudianteId}_${selectedMateria}_${selectedLapso}`;
            const calificacion = prev[key] || { notas: ['', '', '', ''], promedio: 0 };
    ...
    const guardarCalificacion = async (estudianteId) => {
        ...
        try {
            const data = {
                ...
                lapso: selectedLapso,
                nota1: calificacion.notas[0] !== '' ? calificacion.notas[0] : null,
                nota2: calificacion.notas[1] !== '' ? calificacion.notas[1] : null,
                nota3: calificacion.notas[2] !== '' ? calificacion.notas[2] : null,
                nota4: calificacion.notas[3] !== '' ? calificacion.notas[3] : null,
            };

            if (calificacion.id) {
                // Actualizar calificación existente
                await axiosInstance.put(`calificaciones/${calificacion.id}/`, data);
            } else {
            ...
            const errorMsg = error.response?.data?.error || error.response?.data?.detail || 'Error al guardar la calificación';
            alert(errorMsg);
        }
    };

    const enviarCalificacionesFinales = async () => {
        if (!window.confirm('¿Está seguro de enviar las calificaciones finales? Esta acción no se puede deshacer y las calificaciones no podrán ser modificadas.')) {
            return;
        }

        try {
            const response = await axiosInstance.post('calificaciones/enviar_finales/', {
                materia: selectedMateria,
                lapso: selectedLapso
            });
            
            alert(`Calificaciones finales enviadas exitosamente. ${response.data.calificaciones_enviadas || 0} calificaciones fueron enviadas.`);
            
            // Recargar calificaciones para actualizar el estado
            await cargarCalificaciones();
        } catch (error) {
            console.error('Error al enviar calificaciones:', error);
            const errorMsg = error.response?.data?.error || error.response?.data?.detail || 'Error al enviar las calificaciones finales';
            alert(errorMsg);
        }
    };
        ...
        <div className="main-content">
            <div className="header">
                <div className="page-title" style={{width: '100%', textAlign: 'center'}}>
                    <h1 style={{color: 'var(--dark)', margin: 0}}>Registro de Calificaciones</h1>
                    <p style={{color: 'var(--gray)', margin: '8px 0 0 0'}}>Registra y gestiona las calificaciones de tus estudiantes</p>
                </div>
            </div>

            <div className="section-card" style={{maxWidth: '1400px', margin: '0 auto'}}>
                <div style={{
                    display: 'flex',
                    flexWrap: 'wrap',
                    gap: '20px',
                    ...
                    <div style={{flex: '1', minWidth: '250px'}}>
                        <label style={{
                            ...
                            <i className="fas fa-book" style={{marginRight: '6px', color: 'var(--primary)'}}></i>
                            Materia *
                        </label>
                        <select
                    ...
                    <div style={{flex: '1', minWidth: '250px'}}>
                        <label style={{
                            ...
                            <i className="fas fa-calendar-alt" style={{marginRight: '6px', color: 'var(--primary)'}}></i>
                            Lapso *
                        </label>
                        <select
                            value={selectedLapso}
                    ...
                    {selectedMateria && (
                        <div style={{display: 'flex', alignItems: 'flex-end'}}>
                            <button
                                ...
                                onMouseEnter={(e) => {
                                    e.currentTarget.style.background = '#3db8d1';
                        ...
                        <table style={{
                            ...
                            <tbody>
                                {estudiantes.map((estudiante) => {
                                    ...
                                    const calificacion = calificaciones[key] || { notas: ['', '', '', ''], promedio: 0 };
                                    
                                    return (
                                        <tr key={estudiante.id} style={{
                                            borderBottom: '1px solid var(--light-gray)',
                                            transition: 'var(--transition)',
                                            opacity: calificacion.enviado ? 0.7 : 1
                                        }}
                                        onMouseEnter={(e) => {
                                        ...
                                        >
                                            ...
                                            {[0, 1, 2, 3].map((index) => (
                                                <td key={index} style={{
                                                    ...
                                                    <input
                                                        ...
                                                        max="20"
                                                        step="0.01"
                                                        value={calificacion.notas[index] || ''}
                                                        onChange={(e) => handleNotaChange(estudiante.id, index, e.target.value)}
                                                        placeholder="0-20"
                                                        disabled={calificacion.enviado}
                                                        style={{
                                                            width: '80px',
                                                            padding: '8px',
                                                            border: '2px solid var(--light-gray)',
                                                            ...
                                                            textAlign: 'center',
                                                            transition: 'var(--transition)',
                                                            background: calificacion.enviado ? 'var(--light-gray)' : 'white',
                                                            cursor: calificacion.enviado ? 'not-allowed' : 'text'
                                                        }}
                                                        onFocus={(e) => {
                                                            if (!calificacion.enviado) {
                                                                e.currentTarget.style.borderColor = 'var(--primary)';
                                            ...
                                            <td style={{
                                                ...
                                                fontSize: '1rem',
                                                fontWeight: '600',
                                                color: calificacion.promedio >= 10 ? 'var(--success)' : calificacion.promedio > 0 ? 'var(--warning)' : 'var(--gray)'
                                            }}>
                                                {calificacion.promedio > 0 ? calificacion.promedio.toFixed(2) : '—'}
                                            </td>
                                            <td style={{
                                                padding: '15px',
                                                textAlign: 'center'
                                                ...
                                                {calificacion.enviado ? (
                                                    ...
                                                    <button
                                                        onClick={() => guardarCalificacion(estudiante.id)}
                                                        style={{
                                                            padding: '8px 16px',
                                                            background: 'var(--primary)',
                                                            color: 'white',
